...
export { getLocalTodayDateString, getApptDate };
...
export function AppointmentsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTodayAppointments = useCallback(async (showIndicator = false) => {
    if (showIndicator) {
      startGlobalSync();
    }
    try {
      ...
      // Filter by local date string matching today
      const appointmentsList = Array.isArray(data) ? data.filter((a: any) => getApptDate(a) === today) : [];
      console.info(`[AppointmentsContext] [DEBUG] Successfully fetched ${appointmentsList.length} today's appointments from table 'appointments' for date ${today}:`, appointmentsList);
      const sorted = [...appointmentsList].sort((a: Appointment, b: Appointment) => {
  ...
  }, [todayAppointments, notify, fetchTodayAppointments]);
