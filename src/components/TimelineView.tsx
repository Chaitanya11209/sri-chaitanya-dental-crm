...
interface TimelineEvent {
  ...
  type: 'appointment' | 'treatment' | 'prescription' | 'billing' | 'payment' | 'image' | 'case_sheet' | 'consent_form';
  title: string;
  subtitle: string;
  badgeText?: string;
  badgeColor?: string;
  ...
  const allEvents = useMemo(() => {
    ...
    appointments.forEach((appt) => {
      ...
      events.push({
        ...
        subtitle: `Appointment Slot: ${appt.appointment_time || 'General Slot'} with ${appt.doctor_name || 'Dr. Durga Bhavani Jupalli'}`,
    ...
    prescriptions.forEach((rx: any) => {
      ...
      events.push({
        ...
        subtitle: `Prescribed by: ${rx.doctorName || 'Dr. Durga Bhavani Jupalli'}`,
        badgeText: 'Rx Active',
    ...
    if (caseSheets && caseSheets.length > 0) {
      caseSheets.forEach((cs: any) => {
        ...
        events.push({
          ...
          rawDate: dateStr,
          type: 'case_sheet',
          title: `Case Sheet Committed: ${cs.template}`,
          subtitle: `Dentist: ${cs.doctorName || 'Dr. Durga Bhavani Jupalli'}`,
          badgeText: 'CLINICAL CASE',
          badgeColor: 'bg-teal-50 text-teal-700 border-teal-100',
          details: [
            `Chief Complaint: ${cs.chiefComplaint || 'N/A'}`,
            `Clinical Diagnosis: ${cs.diagnosis || 'N/A'}`,
            `Procedure Notes: ${cs.procedureNotes || 'N/A'}`,
            `Materials Used: ${cs.materialsUsed || 'N/A'}`,
            ...(cs.nextVisit ? [`Next Scheduled Visit: ${cs.nextVisit}`] : [])
          ],
    ...
    if (consentForms && consentForms.length > 0) {
      consentForms.forEach((cf: any) => {
        ...
        events.push({
          ...
          rawDate: dateStr,
          type: 'consent_form',
          title: `Consent Authorized: ${cf.formType}`,
          subtitle: `Signed by: ${cf.patientName || 'Patient / Guardian'}`,
          badgeText: 'CONSENT SIGNED',
          badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-100',
          details: [
            `Authorized Treatment: ${cf.formType}`,
            `Date of Consent: ${cf.date}`,
            `Status: Signed & Securely Archived`
          ]
        });
      });
    }
    ...
    return events;
  }, [appointments, treatments, prescriptions, uploadedImages, caseSheets, consentForms, sortOrder]);
