...
import { CLINIC_SIGNATURE, extractPatientFromMessage, getClinicSignature, sanitizeWhatsAppMessage } from '../utils/whatsapp';
...
${sig}`;
  ...
  return sanitizeWhatsAppMessage(msg);
}
...
export async function autoExtractAndSavePatient(
  ...
  try {
    ...
    const code = generateUniquePatientCode();
