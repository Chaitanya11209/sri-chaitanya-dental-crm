...
export async function sendClinicEmail(params: EmailParams): Promise<EmailResponse> {
  ...
  if (!publicKey || publicKey.trim() === '') {
    console.warn('[EMAIL SYSTEM] Dispatch skipped: EmailJS Public Key is missing.');
    return { success: false, error: 'EmailJS Public Key is not configured.' };
  }
  ...
  try {
    ...
    const rawError = typeof err === 'string' 
      ? err 
      : err?.text || err?.message || (typeof err === 'object' ? JSON.stringify(err) : String(err));

    console.warn('[EMAIL SYSTEM] EmailJS dispatch issue:', rawError);

    let cleanError = rawError;
    if (rawError.includes('Public Key is invalid')) {
      cleanError = 'EmailJS Public Key is invalid or missing in environment settings.';
    }
