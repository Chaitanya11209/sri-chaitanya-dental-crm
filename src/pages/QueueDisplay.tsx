...
import { getApptDate } from '../utils/dateUtils';
...
export default function QueueDisplay() {
  ...
  const [upcomingQueue, setUpcomingQueue] = useState<QueueItem[]>([
    { token: 'T04', name: 'Aditya Sharma', doctor: 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)', department: 'Endodontics', room: 'Consulting Cabin 1', waitTime: '12 Mins', status: 'Waiting' },
    ...
    { token: 'T06', name: 'Leela Prasad', doctor: 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)', department: 'Orthodontics', room: 'Cabin 3', waitTime: '35 Mins', status: 'Waiting' },
  ...
  const fetchTodayQueueFromDb = async () => {
    try {
      ...
      const todayData = (data || []).filter((a: any) => getApptDate(a) === todayStr);

      if (todayData && todayData.length > 0) {
        // Find if there's any active patient currently in treatment/consultation
        const activeAppt = todayData.find(
        ...
        if (activeAppt) {
          setNowServing({
            ...
            doctor: activeAppt.doctor_name || 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)',
        ...
        if (waitingAppts.length > 0) {
          const mappedWaiting = waitingAppts.map((a, idx) => ({
            ...
            name: a.name,
            doctor: a.doctor_name || 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)',
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!record || getApptDate(record) !== todayStr) return;
  ...
  const registerMockWalkIn = () => {
    ...
    const consultants = [
      { name: 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)', cab: 'Consulting Cabin 1' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 relative z-10 my-4 flex-1">
          ...
          <div className="bg-slate-950 border-2 border-slate-800 rounded-3xl p-5 space-y-4 md:col-span-2 shadow-inner">
            ...
            <div className="space-y-3 font-sans">
              {[
                { name: 'Dr. Durga Bhavani Jupalli (BDS, Cosmetic Dental Surgeon)', role: 'Chief Prosthodontist', cab: 'Consulting Cabin 1', status: 'Active (T04 Ready)', pulseBg: 'bg-emerald-500/20 text-emerald-450 border-emerald-500/30' },
