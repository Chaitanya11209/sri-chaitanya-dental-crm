...
export default function Reports() {
  ...
  const [activeTab, setActiveTab] = useState<'executive' | 'analytics' | 'daily' | 'monthly' | 'treatment' | 'doctor' | 'patient'>('executive');
  ...
  useEffect(() => {
    ...
    console.info("[Reports] Subscribing to postgres changes channels for 'patients', 'appointments', and 'treatments'.");
    const channelP = supabase
      .channel('reports-patients-realtime')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'patients' },
        () => {
          console.info("[Reports] Patients change detected, refreshing report data.");
          fetchReportData();
        }
      )
      .subscribe();
    ...
    const channelA = supabase
      .channel('reports-appointments-realtime')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'appointments' },
        () => {
          console.info("[Reports] Appointments change detected, refreshing report data.");
          fetchReportData();
        }
      )
      .subscribe();
    ...
    const channelT = supabase
      .channel('reports-treatments-realtime')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'treatments' },
        () => {
          console.info("[Reports] Treatments change detected, refreshing report data.");
          fetchReportData();
        }
      )
      .subscribe();
    ...
    const handleForceSync = () => {
      console.info("[Reports] Force sync triggered. Refreshing report data.");
      fetchReportData();
    };
    ...
    <div className="space-y-6 font-sans text-slate-800">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-3 border-b border-slate-150 bg-white flex flex-wrap gap-1.5">
          {[
            { id: 'executive', label: '👑 Executive BI Command Center' },
        ...
        {/* Actions Row */}
        {activeTab !== 'analytics' && activeTab !== 'executive' && (
