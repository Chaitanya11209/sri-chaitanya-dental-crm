...
import { isAdmin, getRole, canViewFinancials } from '../../lib/auth';
import { 
  Plus, Search, X, Phone, Clock, Calendar, CheckSquare, 
  Send, CheckCircle2, AlertCircle, AlertTriangle, MessageSquare, Edit2, Trash2, HeartPulse, ShieldAlert,
  RefreshCw, Zap, Sparkles
} from 'lucide-react';
import { sendSMS, getSMSTemplates } from '../../lib/sms';
import { CLINIC_SIGNATURE, openWhatsApp, sanitizeWhatsAppMessage, getWhatsAppTemplates, getClinicSignature } from '../../utils/whatsapp';
...
import { validateIndianPhone, normalizeIndianPhone, generateUniquePatientCode } from '../../utils/patientUtils';
...
const TREATMENTS = DENTAL_PROBLEMS;
...
function replaceTokens(template: string, appt: any, defaultDocName = 'Dr. Durga Bhavani Jupalli'): string {
...
export default function Appointments() {
  const admin = isAdmin();
  const role = getRole();
  const canViewFin = canViewFinancials(role as any);
  ...
  const handleDispatchReminder = async (appt: any, type: 'dayBefore' | 'sameDay') => {
    ...
    const defaultDocName = (doctors && doctors[0]?.name) || 'Dr. Durga Bhavani Jupalli';
  ...
  const handleApproveCancellation = async (appt: any) => {
    const confirmApprove = window.confirm(`Are you sure you want to approve the cancellation request for ${appt.name}? This will mark the appointment as Cancelled.`);
    if (!confirmApprove) return;

    try {
      const cleanedNotes = cleanNotes(appt.notes);
      
      // Update the DB with Cancelled status and cleaned notes
      const { error } = await supabase
        .from('appointments')
        .update({ status: 'Cancelled', notes: cleanedNotes })
        .eq('id', appt.id);
        
      if (error) throw error;

      // Optimistic update local appointments state
      setAppointments(prev => prev.map(a => a.id === appt.id ? { ...a, status: 'Cancelled', notes: cleanedNotes } : a));

      // Trigger standard cancellation side-effects (waiting list reallocation, huddles, etc.)
      setVacantSlotNotification({
        id: appt.id,
      ...
      await syncPatientStatusByAppointment(appt.id);
      notify('success', 'Cancellation Approved', `Appointment for ${appt.name} has been cancelled.`);
      fetch();
    } catch (err: any) {
      console.error(err);
      notify('error', 'Approval Failed', 'Failed to approve cancellation request.');
    }
  };

  const handleRejectCancellation = async (appt: any) => {
    const confirmReject = window.confirm(`Are you sure you want to decline the cancellation request for ${appt.name}? This will clear the cancellation request flag and keep the appointment active.`);
    if (!confirmReject) return;

    try {
      const cleanedNotes = cleanNotes(appt.notes);
      
      // Update the DB with cleaned notes (removing the cancellation flag)
      const { error } = await supabase
        .from('appointments')
      ...
      // Optimistic update local appointments state
      setAppointments(prev => prev.map(a => a.id === appt.id ? { ...a, notes: cleanedNotes } : a));

      notify('info', 'Cancellation Declined', `Cancellation request for ${appt.name} has been declined. Appointment remains active.`);
      fetch();
    } catch (err: any) {
      console.error(err);
      notify('error', 'Decline Failed', 'Failed to decline cancellation request.');
    }
  };
  ...
  const handleAssignSlot = async (candidate: any) => {
    ...
    const normalizedPhone = normalizeIndianPhone(candidate.phone);
  ...
  useEffect(() => {
    ...
    const handleScheduledReminders = () => {
      ...
      appointments.forEach(a => {
        ...
        if (getApptDate(a) === todayString && a.appointment_time && a.status === 'Confirmed') {
...
Sri Chaitanya Dental Care`;
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Search matching patient record via exact normalized phone or clean last 10 digits
      ...
      if (!form.appointment_time || !isValidAppointmentTime(form.appointment_time)) {
  ...
  const filtered = appointments.filter(a => {
    ...
    const matchDate = todayOnly ? (getApptDate(a) === todayString) : (!dateFilter || getApptDate(a) === dateFilter);
  ...
  const sortedFiltered = [...filtered].sort((a, b) => {
    ...
    if (sortBy === 'patient_name') {
      ...
      const timeMinsA = parseTimeToMinutes(a.appointment_time);
      const timeMinsB = parseTimeToMinutes(b.appointment_time);
      if (timeMinsA < timeMinsB) return sortOrder === 'asc' ? -1 : 1;
      if (timeMinsA > timeMinsB) return sortOrder === 'asc' ? 1 : -1;
      return 0;
    }
                    ...
                    return (
                      <div key={appt.id} className={`p-3.5 rounded-xl border flex flex-col sm:flex-row items-center justify-between gap-3 text-xs transition duration-350 font-sans ${isDue ? 'bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-slate-900/40 border-amber-500/45 shadow-lg shadow-amber-500/5 ring-1 ring-amber-500/20' : 'bg-slate-900/60 border-white/5 hover:border-slate-700'}`}>
                        <div className="space-y-0.5 text-left w-full sm:w-auto">
                          ...
                          <p className="text-[10px] text-slate-350">{appt.treatment} with {appt.doctor_name || 'Dr. Durga Bhavani Jupalli'}</p>
                          ...
                          <div className="flex items-center gap-1 bg-slate-950 p-1.5 rounded-lg border border-white/5">
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                const doc = appt.doctor_name || 'Dr. Durga Bhavani Jupalli';
                                const msg = replaceTokens(rawStr, appt, doc);
                                openWhatsApp(appt.phone, msg);
                  ...
                  {sortedFiltered.map(a => (
                    <tr key={a.id} className={`hover:bg-slate-50/50 transition-colors ${selectedIds.includes(a.id) ? 'bg-teal-50/10' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <p className="font-bold text-slate-800 text-sm hover:text-teal-600 cursor-pointer transition flex items-center gap-1.5" onClick={() => handleOpenEditModal(a)}>
                          {a.name}
                          {isCancelRequested(a.notes) && (
                            <span className="bg-rose-100 text-rose-700 text-[9.5px] px-1.5 py-0.5 rounded font-bold animate-pulse inline-flex items-center gap-0.5" title="Patient requested cancellation for this appointment">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <p className="font-semibold text-slate-700 text-xs">{a.doctor_name || (doctors && doctors[0]?.name) || 'Dr. Durga Bhavani Jupalli'}</p>
                        </div>
                      </td>
                          ...
                          {isCancelRequested(a.notes) && (
                            <div className="flex gap-1.5">
                              <button
                                ...
                                className="bg-rose-600 hover:bg-rose-700 text-white text-[9.5px] font-bold px-1.5 py-0.5 rounded-md transition-all cursor-pointer shadow-3xs"
                              ...
                              <button
                                ...
                                className="bg-slate-200 hover:bg-slate-300 text-slate-705 text-[9.5px] font-bold px-1.5 py-0.5 rounded-md transition-all cursor-pointer"
                ...
                <div key={a.id} className={`p-4 flex gap-3 items-start transition-colors ${selectedIds.includes(a.id) ? 'bg-teal-50/20' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-bold text-slate-800 text-sm hover:text-teal-600 transition flex items-center gap-1.5 flex-wrap" onClick={() => handleOpenEditModal(a)}>
                          {a.name}
                          {isCancelRequested(a.notes) && (
                            <span className="bg-rose-100 text-rose-700 text-[9px] px-1 py-0.5 rounded font-black animate-pulse inline-flex items-center gap-0.5">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    <p className="text-[11px] text-slate-500 font-bold">{a.doctor_name || (doctors && doctors[0]?.name) || 'Dr. Durga Bhavani Jupalli'}</p>
                  </div>
                  {canViewFin && (
                    ...
                    {isCancelRequested(a.notes) && (
                      <>
                        <button
                          type="button"
                          onClick={() => handleApproveCancellation(a)}
                          className="text-xs text-rose-600 hover:underline font-extrabold flex items-center gap-0.5"
                        >
                          Approve Cancel
                        </button>
                        <button
                          type="button"
                          onClick={() => handleRejectCancellation(a)}
                          className="text-xs text-slate-500 hover:underline font-semibold flex items-center gap-0.5"
                        >
                          Decline Cancel
                        </button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-xl animate-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <form onSubmit={save} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs font-mono font-semibold focus:outline-none focus:ring-1 focus:ring-teal-500"
                  />
                </div>
              </div>
              ...
              <ReasonForVisitSelect
                ...
                onChange={val => setForm(f => ({ ...f, treatment: val }))}
                required
                label="Treatment Procedure / Reason for Visit *"
              />
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    required
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-teal-500 bg-white"
                  >
            ...
            <div className="bg-slate-900 px-6 py-4 flex items-center justify-between text-white flex-shrink-0">
              <div className="flex items-center gap-2.5 text-left">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold font-sans">Sri Chaitanya Multispeciality Dental Care Portal</p>
                </div>
              </div>
              {!bulkSending && (
                <button 
...
function constructWhatsAppMessage(alerts: any): string {
  const heading = alerts.status === 'Rescheduled' ? 'Appointment Rescheduled' : 'Appointment Confirmed';
  const sig = getClinicSignature();
  const rawMsg = `${heading}

Patient: ${alerts.patientName}
Doctor: ${alerts.doctorName}
Date: ${alerts.date}
Time: ${alerts.time || '04:00 PM'}

${sig}`;

  return sanitizeWhatsAppMessage(rawMsg);
}
