...
import { normalizeIndianPhone } from '../../utils/patientUtils';
...
export default function Treatments() {
  ...
  const [selectedDoctorName, setSelectedDoctorName] = useState('Dr. Durga Bhavani Jupalli');
  ...
  const [pdfPlanForm, setPdfPlanForm] = useState<any>({
    patient_name: '',
    phone: '',
    doctorName: 'Dr. Durga Bhavani Jupalli',
  ...
  const generatePDFPlan = async (plan: any) => {
    try {
      ...
      doc.text([
        `Attending Specialist: ${plan.doctorName || 'Dr. Durga Bhavani Jupalli'}`,
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      const normalizedPhone = form.phone ? normalizeIndianPhone(form.phone) : '';
      const payload = {
        ...form,
      ...
      if (error) {
        console.error('[Treatments] Error saving treatment:', error);
        alert(`Failed to save treatment record: ${error.message}`);
      } else {
        setShowModal(false);
      ...
      console.error('[Treatments] Exception during save:', err);
    }
    setSaving(false);
  };
  ...
  const updateStage = async (id: any, stage: string) => {
    ...
    try {
      const { error } = await supabase.from('treatments').update({ stage }).eq('id', id);
      if (error) {
        console.error('[Treatments] Error updating stage:', error);
        alert(`Failed to update treatment stage: ${error.message}`);
      } else {
        fetch();
      }
    } catch (err: any) {
      console.error('[Treatments] Exception during updateStage:', err);
    }
  };
  ...
  const updateSessions = async (id: any, done: number, total: number) => {
    ...
    try {
      ...
      if (error) {
        console.error('[Treatments] Error updating sessions:', error);
        alert(`Failed to update session counts: ${error.message}`);
      } else {
        fetch();
      }
    } catch (err: any) {
      console.error('[Treatments] Exception during updateSessions:', err);
    }
  };
