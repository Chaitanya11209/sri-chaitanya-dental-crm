...
import { useBillingRealtime, usePatientsRealtime } from '../../hooks/useRealtimeHooks';
...
export default function Billing() {
  ...
  useEffect(() => {
    if (hookPatients) {
      console.info("[Billing] [UI] Reacting to updated realtime patients list from hook.");
      const sorted = [...hookPatients].sort((a: any, b: any) => (a.name || '').localeCompare(b.name || ''));
  ...
  const generatePDF = async (a: any) => {
    try {
      ...
      doc.text(`Attending Surgeon: ${a.doctor_name || 'Dr. Durga Bhavani Jupalli'}`, 15, 68);
  ...
  const parseBilling = (appt: any) => {
    ...
    return {
      ...
      follow_up_date: '',
      instructions: '',
      doctor_name: appt.doctor_name || 'Dr. Durga Bhavani Jupalli',
