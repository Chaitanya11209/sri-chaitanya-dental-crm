...
export default function AuditLogs() {
  ...
  const fetchAuditLogs = async () => {
    ...
    try {
      ...
      // If we have some data, use it; otherwise merge with local logs for maximum fidelity
      const localLogsStr = localStorage.getItem('scdc_local_audit_logs');
      const localLogs = localLogsStr ? JSON.parse(localLogsStr) : [];
      
      const remoteLogs = data || [];
      const mergedLogs = [...remoteLogs];
      
      // Add local logs that are not already present (by matching content/time or just appending)
      localLogs.forEach((localLog: any) => {
        const alreadyExists = remoteLogs.some(
          (remoteLog: any) => 
            remoteLog.action === localLog.action && 
            remoteLog.created_at === localLog.created_at
        );
        if (!alreadyExists) {
      ...
      // Sort merged logs by created_at descending
      mergedLogs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setLogs(mergedLogs.slice(0, 100));
    } catch (err) {
      console.warn('Supabase audit logs read failed, falling back to offline browser storage.', err);
      try {
        const localLogsStr = localStorage.getItem('scdc_local_audit_logs');
        const localLogs = localLogsStr ? JSON.parse(localLogsStr) : [];
        setLogs(localLogs);
