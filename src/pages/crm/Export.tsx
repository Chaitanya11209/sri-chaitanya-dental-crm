...
import { runSystemDataDiagnostics, type SystemDiagnosticReport } from '../../lib/diagnostics';
import { insertPatientsWithUniqueCode } from '../../utils/patientUtils';
...
export default function Export() {
  ...
  // Diagnostics & Reconciliation state
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      const report = await runSystemDataDiagnostics();
      setDiagnosticReport(report);
      if (report.summary.status === 'HEALTHY') {
        notify('success', 'Diagnostics Passed', 'No financial balance mismatches or patient data integrity issues detected.');
      } else {
        notify('warning', 'Discrepancies Found', `Detected ${report.summary.financialDiscrepanciesCount} financial mismatches and ${report.summary.patientDiscrepanciesCount} patient integrity issues.`);
      }
    } catch (err: any) {
      notify('error', 'Diagnostics Failed', err?.message || String(err));
    } finally {
  ...
  const generateZIPInBrowser = async (zipType: 'all' | 'source' | 'database' | 'production' | 'master') => {
    // 1. Fetch EXPORT_MANIFEST.json
    const manifestRes = await fetch('/EXPORT_MANIFEST.json');
    if (!manifestRes.ok) {
      throw new Error('Could not load workspace export manifest.');
    }
    ...
    // 3. Loop and fetch every single file, adding it to the zip
    for (const item of manifest) {
      const relPath = item["full path"];
      
      // Filter based on zipType (matching the backend pack-project.js filter criteria)
      if (zipType === 'database') {
        const isDbFile = relPath.startsWith('supabase') || relPath === '.env.example' || relPath === 'DEPLOY_GUIDE.md' || relPath === 'README.md';
        if (!isDbFile) continue;
      }
      
      try {
        const fileContentRes = await fetch('/' + relPath);
        if (fileContentRes.ok) {
          const blob = await fileContentRes.blob();
          zip.file(relPath, blob);
        } else {
          console.warn(`Could not fetch file: ${relPath}`);
        }
      } catch (err) {
        console.warn(`Error fetching file to bundle: ${relPath}`, err);
      }
    }
  ...
  const handleDownloadZIP = async () => {
    ...
    setMessage(null);
    try {
      notify('info', 'Compiling Live Workspace ZIP', 'Gathering fully up-to-date files from workspace...');
      const blob = await generateZIPInBrowser('all');
      ...
      setMessage({ type: 'success', text: 'Project ZIP codebase compiled on-the-fly and downloaded successfully!' });
      ...
      try {
        ...
        a.click();
        document.body.removeChild(a);
        setMessage({ type: 'success', text: 'Backup download triggered successfully (fallback ZIP).' });
  ...
  const handleDownloadSourceCode = async () => {
    ...
    notify('info', 'Compiling Source Code', 'Gathering up-to-date source files...');
    try {
      const blob = await generateZIPInBrowser('source');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'sri-chaitanya-dental-crm-source.zip';
      ...
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      notify('success', 'Source Code Ready', "'sri-chaitanya-dental-crm-source.zip' compiled and downloaded successfully.");
    } catch (err: any) {
  ...
  const handleDownloadDatabaseZIP = async () => {
    ...
    notify('info', 'Compiling Database Schema', 'Structuring up-to-date schemas, migrations, policies and triggers...');
    try {
      const blob = await generateZIPInBrowser('database');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      ...
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      notify('success', 'Database Package Ready', "'sri-chaitanya-dental-crm-database.zip' compiled and downloaded successfully.");
    } catch (err: any) {
  ...
  const handleDownloadDeploymentPackage = async () => {
    ...
    notify('info', 'Compiling Deployment Bundle', 'Preparing up-to-date migrations, schemas, and configurations...');
    try {
      const blob = await generateZIPInBrowser('production');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      ...
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      notify('success', 'Deployment Package Complete', "'sri-chaitanya-dental-crm-production-package.zip' compiled and downloaded successfully.");
    } catch (err: any) {
  ...
  const handleDownloadMasterRepository = async () => {
    ...
    try {
      const blob = await generateZIPInBrowser('master');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
    ...
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <form onSubmit={(e) => {
                ...
                />
                <button type="submit" className="px-3 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-xl text-xs flex-shrink-0 transition cursor-pointer">
                  Apply
                </button>
              </form>
              <p className="text-[10px] text-slate-500 leading-normal">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 text-[10px] font-extrabold uppercase tracking-wide rounded">Admin Center</span>
                  <h3 className="text-slate-850 font-extrabold text-base tracking-tight flex items-center gap-2">
                    <FileSpreadsheet className="text-teal-600" size={18} /> DATA EXPORT (Operational CSV / JSON)
                  </h3>
                </div>
                <p className="text-slate-500 text-xs">Export clinical operational records (Patients, Appointments, Treatments, Invoices) as CSV or JSON for offline reporting.</p>
              </div>
              <button 
                onClick={handleExportAllData}
                disabled={exportingAll}
                className="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 font-bold rounded-xl text-xs flex items-center gap-1.5 transition active:scale-95 shadow-sm border-none disabled:opacity-50 cursor-pointer"
              >
                {exportingAll ? (
          ...
          {/* Financial Reconciliation & Patient Data Integrity Diagnostic Suite */}
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-slate-850 font-extrabold text-base tracking-tight flex items-center gap-2">
                  <ShieldCheck className="text-teal-600" size={18} /> Financial Reconciliation & Data Integrity Diagnostics
                </h3>
                <p className="text-slate-500 text-xs mt-0.5">Scan database records for invoice balance mismatches, unlinked payments, orphan appointments, and patient code issues.</p>
              </div>
              <button
                onClick={handleRunDiagnostics}
                disabled={runningDiagnostics}
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition active:scale-95 disabled:opacity-50 cursor-pointer"
              >
                {runningDiagnostics ? <RefreshCw size={13} className="animate-spin" /> : <Activity size={13} className="text-teal-400" />}
                {runningDiagnostics ? 'Scanning DB...' : 'Run Diagnostics'}
              </button>
            </div>
              ...
              <div className="space-y-4 text-xs animate-fadeIn">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <p className="text-[10px] text-slate-500 font-bold uppercase">Invoices Audited</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">{diagnosticReport.summary.totalInvoicesChecked}</p>
                  </div>
                  <div className={`p-3 rounded-xl border ${diagnosticReport.summary.financialDiscrepanciesCount === 0 ? 'bg-emerald-50/50 border-emerald-100 text-emerald-800' : 'bg-rose-50/50 border-rose-100 text-rose-800'}`}>
                    <p className="text-[10px] font-bold uppercase">Financial Mismatches</p>
                    <p className="text-base font-extrabold mt-1">{diagnosticReport.summary.financialDiscrepanciesCount}</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <p className="text-[10px] text-slate-500 font-bold uppercase">Patients Audited</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">{diagnosticReport.summary.totalPatientsChecked}</p>
                  </div>
                  <div className={`p-3 rounded-xl border ${diagnosticReport.summary.patientDiscrepanciesCount === 0 ? 'bg-emerald-50/50 border-emerald-100 text-emerald-800' : 'bg-amber-50/50 border-amber-100 text-amber-800'}`}>
                    <p className="text-[10px] font-bold uppercase">Patient Code Issues</p>
                    <p className="text-base font-extrabold mt-1">{diagnosticReport.summary.patientDiscrepanciesCount}</p>
                  </div>
                </div>

                {diagnosticReport.financialIssues.length > 0 && (
                  <div className="space-y-2">
                    <p className="font-bold text-rose-700 flex items-center gap-1.5 text-xs">
                      <AlertCircle size={14} /> Financial Discrepancies ({diagnosticReport.financialIssues.length})
                    </p>
                    <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1 font-mono text-[11px]">
                      {diagnosticReport.financialIssues.map((issue) => (
                        <div key={issue.id} className="p-2.5 rounded-lg bg-rose-50 border border-rose-150 text-rose-900 leading-relaxed">
                          {issue.details}
                        </div>
                      ))}
                    </div>
                ...
                {diagnosticReport.patientIssues.length > 0 && (
                  <div className="space-y-2">
                    <p className="font-bold text-amber-700 flex items-center gap-1.5 text-xs">
                      <AlertCircle size={14} /> Patient Data Integrity Issues ({diagnosticReport.patientIssues.length})
                    </p>
                    <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1 font-mono text-[11px]">
                      {diagnosticReport.patientIssues.map((issue) => (
                        <div key={issue.id} className="p-2.5 rounded-lg bg-amber-50 border border-amber-150 text-amber-900 leading-relaxed">
                          {issue.details}
                        </div>
                      ))}
                    </div>
                ...
                {diagnosticReport.summary.status === 'HEALTHY' && (
                  <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center gap-2">
                    <CheckCircle2 size={16} className="text-emerald-600 flex-shrink-0" />
                    <span className="font-semibold">All active invoices and patient records passed non-destructive reconciliation checks!</span>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Infrastructure Database Backup Strategy & Recovery Procedures */}
          <div className="bg-slate-900 text-white rounded-2xl border border-slate-800 p-6 shadow-xl space-y-4">
            <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
              <div className="w-9 h-9 rounded-xl bg-teal-950 border border-teal-800/80 flex items-center justify-center text-teal-400">
                <Database size={18} />
              </div>
              <div>
                <h3 className="font-extrabold text-sm text-teal-400">INFRASTRUCTURE DATABASE BACKUP & RECOVERY POLICY</h3>
                <p className="text-slate-400 text-xs mt-0.5">Automated Supabase Cloud snapshots, Write-Ahead Logs (WAL), and point-in-time restoration SOPs.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1">
                <p className="text-[10px] text-teal-400 font-bold uppercase">Automated Schedule</p>
                <p className="font-semibold text-slate-200">Daily WAL Archiving</p>
                <p className="text-[11px] text-slate-400 leading-relaxed">Continuous physical database log streaming to encrypted cloud storage.</p>
              </div>
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1">
                <p className="text-[10px] text-teal-400 font-bold uppercase">Retention SLA</p>
                <p className="font-semibold text-slate-200">7 to 30 Days Rolling</p>
                <p className="text-[11px] text-slate-400 leading-relaxed">Automated rolling snapshots with zero-downtime point-in-time recovery capability.</p>
              </div>
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1">
                <p className="text-[10px] text-teal-400 font-bold uppercase">Recovery RTO / RPO</p>
                <p className="font-semibold text-slate-200">RPO &lt; 5 mins | RTO &lt; 15 mins</p>
                <p className="text-[11px] text-slate-400 leading-relaxed">Minimal data loss tolerance for clinical records and financial invoices.</p>
              </div>
            </div>

            <div className="bg-slate-950 rounded-xl p-4 border border-slate-800 space-y-2 text-xs">
              <p className="font-bold text-slate-200 flex items-center gap-1.5">
                <ShieldCheck size={14} className="text-teal-400" /> Disaster Recovery Procedure (Admin SOP)
              </p>
              <ol className="list-decimal list-inside text-slate-400 space-y-1 text-[11px] leading-relaxed font-mono">
                <li>Log in to the Supabase Cloud Management Console for the Sri Chaitanya project.</li>
                <li>Navigate to <strong className="text-slate-200">Project Settings &gt; Database &gt; Backups</strong>.</li>
                <li>Select the desired snapshot timestamp or specify Point-In-Time Recovery (PITR) minutes.</li>
                <li>Click <strong className="text-slate-200">Restore Snapshot</strong> to spin up the target recovery database.</li>
                <li>Update <code className="bg-slate-900 px-1 py-0.5 rounded text-teal-300">VITE_SUPABASE_URL</code> and <code className="bg-slate-900 px-1 py-0.5 rounded text-teal-300">VITE_SUPABASE_ANON_KEY</code> in environment settings.</li>
              </ol>
            </div>
          </div>
                    ...
                    <div className="space-y-1.5 pt-1.5 border-t border-slate-200/50 text-slate-800">
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          className="w-full bg-white border border-slate-200 text-xs rounded-lg py-1.5 pl-8 pr-3 outline-none focus:border-sky-500 cursor-pointer font-medium text-slate-700"
                        >
