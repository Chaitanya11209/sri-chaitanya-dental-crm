...
if (typeof window !== 'undefined') {
  ...
  if (cached) {
    if (cached.includes('\uFFFD') || cached.includes('') || !cached.includes(clinicConfig.clinicName)) {
      localStorage.removeItem('clinic_signature');
      CLINIC_SIGNATURE = DEFAULT_SIGNATURE;
    } else {
...
export function sanitizeWhatsAppMessage(msg: string): string {
  ...
  // 1. Remove Unicode replacement character \uFFFD () if present
  let cleaned = msg.replace(/\uFFFD/g, '');

  // 2. Map known currencies, smart quotes, dashes, and zero-width spaces to clean equivalents
  cleaned = cleaned
  ...
  // 3. Remove any remaining isolated surrogate code units (0xD800 to 0xDFFF without its pair)
  // This prevents encodeURIComponent from failing or outputting \uFFFD ()
