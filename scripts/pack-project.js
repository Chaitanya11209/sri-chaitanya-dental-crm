...
export async function generateZIP(customZipType = 'all') {
  ...
  if (customZipType === 'database') {
    ...
    const outPath = path.join(backupsDir, 'sri-chaitanya-dental-crm-database.zip');
    ...
    const outPath = path.join(backupsDir, 'sri-chaitanya-dental-crm-source.zip');
    fs.writeFileSync(outPath, content);
    ...
    const outPath = path.join(backupsDir, 'sri-chaitanya-dental-crm-production-package.zip');
    fs.writeFileSync(outPath, content);
    ...
    const outPath = path.join(backupsDir, 'Sri-Chaitanya-Dental-CRM-Full-Repository.zip');
    fs.writeFileSync(outPath, content);
    ...
    fs.writeFileSync(path.join(backupsDir, 'project-export.zip'), content);
    fs.writeFileSync(path.join(backupsDir, 'sri-chaitanya-dental-crm-source.zip'), content);
    fs.writeFileSync(path.join(backupsDir, 'sri-chaitanya-dental-crm-production-package.zip'), content);
    fs.writeFileSync(path.join(backupsDir, 'Sri-Chaitanya-Dental-CRM-Full-Repository.zip'), content);

    // Also write database zip specifically
    await generateZIP('database');

    console.log(`[EXPORTER] All targets successfully bundled and written to ${backupsDir}`);
